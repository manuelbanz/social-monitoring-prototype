 ...
 *
 * Zend_Controller_Action_Helper_ViewRenderer provides transparent htdocs
 * integration for action controllers. It allows you to create a htdocs object
 * once, and populate it throughout all actions. Several global options may be
 * set:
 *
 * - noController: if set true, render() will not look for htdocs scripts in
 *   subdirectories named after the controller
 * - viewSuffix: what htdocs script filename suffix to use
 *
 * The helper autoinitializes the action controller htdocs preDispatch(). It
 * determines the path to the class file, and then determines the htdocs base
 ...
 * $viewHelper = $this->_helper->getHelper('htdocs');
 *
...
class Zend_Controller_Action_Helper_ViewRenderer extends Zend_Controller_Action_Helper_Abstract
{
    ...
    /**
     * Which action htdocs script to render
     * @var string
     */
     ...
     * Constructor
     *
     * Optionally set htdocs object and options.
     *
    ...
    /**
     * Clone - also make sure the htdocs is cloned.
     *
     * @return void
     */
    ...
    /**
     * Initialize the htdocs object
     *
     * $options may contain the following keys:
     * - neverRender - flag dis/enabling postDispatch() autorender (affects all subsequent calls)
     * - noController - flag indicating whether or not to look for htdocs scripts in subdirectories named after the controller
     * - noRender - flag indicating whether or not to autorender postDispatch()
     * - responseSegment - which named response segment to render a htdocs script to
     * - scriptAction - what action script to render
     * - viewBasePathSpec - specification to use for determining htdocs base path
     * - viewScriptPathSpec - specification to use for determining htdocs script paths
     * - viewScriptPathNoControllerSpec - specification to use for determining htdocs script paths when noController flag is set
     * - viewSuffix - what htdocs script filename suffix to use
     *
     * @param  string $path
     * @param  string $prefix
    ...
    public function initView($path = null, $prefix = null, array $options = array())
    {
        ...
        if (empty($path)) {
            ...
            if (empty($path)) {
                 ...
                 */
                require_once 'Zend/Controller/Action/Exception.php';
                throw new Zend_Controller_Action_Exception('ViewRenderer initialization failed: retrieved htdocs base path is empty');
            }
        }
        ...
        // Register htdocs with action controller (unless already registered)
    ...
    /**
     * Set htdocs basePath specification
     *
    ...
    /**
     * Retrieve the current htdocs basePath specification string
     *
     * @return string
     */
    ...
    /**
     * Set htdocs script path specification
     *
     * Specification can contain one or more of the following:
     * - :moduleDir - current module directory
     * - :controller - name of current controller in the request
    ...
    /**
     * Retrieve the current htdocs script path specification string
     *
     * @return string
     */
    ...
    /**
     * Set htdocs script path specification (no controller variant)
     *
     * Specification can contain one or more of the following:
     * - :moduleDir - current module directory
     * - :controller - name of current controller in the request
    ...
    /**
     * Retrieve the current htdocs script path specification string (no controller variant)
     *
     * @return string
     */
    ...
    /**
     * Get a htdocs script based on an action and/or other variables
     *
    ...
    /**
     * Set options for rendering a htdocs script
     *
     ...
     * - :suffix - htdocs script file suffix
     *
     * @param  array $vars
     * @return string
     */
    ...
    /**
     * Render a htdocs script (optionally to a named response segment)
     *
    ...
    /**
     * Render a htdocs based on path specifications
     *
     * Renders a htdocs based on the htdocs script path specifications.
     *
     * @param  string  $action
     * @param  string  $name
     ...
     *
     * Pass an action, and one or more specification variables (htdocs script suffix)
     * to determine the htdocs script path, and render that script.
     *
     * @param  string $action
     * @param  array  $vars
     * @param  string $name
    ...
    /**
     * postDispatch - auto render a htdocs
     *
     * Only autorenders if:
     * - _noRender is false
    ...
    /**
     * Should the ViewRenderer render a htdocs script?
     *
     * @return boolean
     */
