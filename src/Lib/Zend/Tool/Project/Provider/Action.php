    ...
    implements Zend_Tool_Framework_Provider_Pretendable
{
     ...
     * @param bool $viewIncluded     Whether the htdocs should the htdocs be included.
    ...
    public function create($name, $controllerName = 'Index', $viewIncluded = true, $module = null)
    {
        ...
        if ($viewIncluded) {
            ...
            if ($this->_registry->getRequest()->isPretend()) {
                $response->appendContent(
                    'Would create a htdocs script for the ' . $name . ' action method at ' . $viewResource->getContext()->getPath()
                    );
            } else {
                $response->appendContent(
                    'Creating a htdocs script for the ' . $name . ' action method at ' . $viewResource->getContext()->getPath()
                    );
                $viewResource->create();
                $this->_storeProfile();
            }
