...
/**
 * Concrete class for handling htdocs scripts.
 *
 * @category   Zend
 * @package    Zend_View
 * @copyright  Copyright (c) 2005-2010 Zend Technologies USA Inc. (http://www.zend.com)
...
class Zend_View extends Zend_View_Abstract
{
    ...
    public function __construct($config = array())
    {
        ...
        if ($this->_useViewStream) {
            if (!in_array('zend.htdocs', stream_get_wrappers())) {
                require_once 'Zend/View/Stream.php';
                stream_wrapper_register('zend.htdocs', 'Zend_View_Stream');
            }
        }
    ...
    /**
     * Includes the htdocs script in a scope with only public $this variables.
     *
     * @param string The htdocs script to execute.
     */
    protected function _run()
    {
        if ($this->_useViewStream && $this->useStreamWrapper()) {
            include 'zend.htdocs://' . func_get_arg(0);
        } else {
